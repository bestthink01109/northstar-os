...
BUN_CEO
  └── COO（Codex/Claude Code/Antigravity）
        ...
        │     ├── SALES: sales_director_v1 / russell_brunson_sales_flow_v1 / satou_masahiro_practical_sales_v1
        │     │          brian_tracy_closing_discipline_v1
...
## AI Runner状態（2026-06-01時点）

| AI | Status |
|----|--------|
| Codex（GPT-5.4） | 明日復帰予定 |
| Claude Code | runner 稼働中（board_runner_monitor.sh / launchd）|
| Antigravity | limited |
| AUDIT Director | 自動実行（crontab・独立稼働）|

### Runner 自動切り替え（Heartbeat ファイル方式）
- 制御ファイル: `Vault/logs/runner_owner.json`
- CODEX 復帰時: `owner: codex` + `expires_at` を書き込むだけで Claude Code runner が自動待機
