...
| 2026/05/07 | v3.3 | 秋吉AI方式適用：Signal DB・Story DB設計追加・BUN社長デジタルクローン原則追加・会議前ブリーフィング・LINE会話自動記録化・リアルタイム戦略更新フロー追加 |
...
### BUN社長デジタルクローン原則（秋吉AI方式適用）

COOはBUN社長の意思決定パターンをエミュレートする「デジタルクローン機能」を持つ。各部門やCOO自身が社長判断を必要とする場面で、COOがBUN社長の回答を代行することで、社長への会議・相談コストを大幅に削減する。BUN社長が直接AIに「自分の判断ロジック」を食わせているため、外部のエンジニアが作るシステムより精度が高い。
...
**行動・対応の優先順位**
- 収益に直結するOPS自動化 → 最優先
- 介護・福祉業界の法令・加算への対応 → 次点（顧客への信頼維持）
- 新規事業（BizDev・プロダクト開発） → OPSが安定した後
- 社内効率化（コスト削減・時間削減） → 常に意識するが単独の目的にしない

**BUN社長の行動パターン（シグナル→アクション変換原則）**
- 異業界のプロフェッショナルとの出会いを「確変トリガー」として重視する
- 「自分一人でできないこと」を専門家に委ねてレバレッジをかける
- 一度成功したパターンは繰り返す。失敗パターンは記録して避ける
- 意思決定はスピード優先。完璧を待たずに動く
- 顧客（介護施設・福祉施設）の現場スタッフ目線を常に忘れない

**クローン機能の運用ルール**
- COOは各部門からの相談に対して「BUN社長ならどう判断するか」をStory DBおよびクローン原則を参照して仮回答を生成する
- お金・契約・OPS現状判断・大きな方向性は必ず社長にエスカレーション（絶対ルール遵守）
- クローン精度はBUN社長とCOOのLINE対話が蓄積されるほど向上する（Signal DB→Story DBサイクル）
...
| Signal DB判定・記録 | Gemini 2.5 Flash | Google Gemini account | HTTP Request（Google AI API） | $0.30 / $2.50 | LINE会話・イベントからのシグナル抽出 |
| 会議前ブリーフィング生成 | Claude Sonnet 4.6 | Anthropic account | HTTP Request（Anthropic API） | $3 / $15 | Story DB参照・ブリーフィング生成 |
| リアルタイム戦略評価 | Claude Opus 4.6 | Anthropic account | HTTP Request（Anthropic API） | $5 / $25 | 事業イベントの戦略的影響評価 |
...
- Signal DB判定・LINE会話記録 → Gemini 2.5 Flash（大量・高頻度処理のためコスト最適）
- 会議前ブリーフィング → Claude Sonnet 4.6（Story DB参照・文脈理解が必要）
- リアルタイム戦略評価 → Claude Opus 4.6（経営的影響判断。妥協不可）
...
| 厚労省 | 1--XvZsAKB1R7jqhRC0bZvvtxbD-7XvhT |
...
#### Signal DB設計（秋吉AI方式・構造化版）

現在のシグナル検知フローを拡張し、検知されたシグナルを「意思決定に役立つ情報の倉庫」として体系的に蓄積するSignal DBを構築する。シグナルを1日50件程度・週400件規模で蓄積し、後述のStory DBの材料とする。

**Signal DBの構造（Google Drive / Reports / BizDev / Signal_DB.csv）**

| フィールド | 内容 |
|---------|------|
| 日時 | シグナル検知日時（YYYY-MM-DD HH:MM） |
| ソース | LINE / カレンダー / RSC / OPS / BizDev |
| ランク | S / A / B / C |
| シグナル本文 | 抽出した意味ある情報（1〜3行） |
| アクション状態 | 採用済み / 検討中 / 却下 / 未対応 |
| 関連ストーリーID | Story DBとのリンク（空欄可） |

**n8n実装方針**
- 既存のシグナル検知フローの出力をLINE通知に加え、Signal_DB.csvへの同時書き込みに拡張する
- BUN社長とCOOのLINEやり取りも自動でSignal DBに記録する（LINE会話自動記録化フロー参照）
- 週次バッチでSignal DBをクラスタリング分析し、Story DB昇格候補をBUN社長にLINEで提案する

#### Story DB設計（秋吉AI方式・BUN社長版）

Signal DB（点）を時間軸でつなげて「線（ストーリー）」に変換し、COOがBUN社長の意思決定を支援する際の参照軸とするデータベース。経営者の意思決定は個別情報ではなく成功・失敗の文脈（ストーリー）に基づくという秋吉AI方式の核心。

**Story DBの構造（GitHub / northstar-os / Story_DB.md）**

| フィールド | 内容 |
|---------|------|
| ストーリーID | S001, S002 ... |
| タイトル | ストーリーを一言で表す（例:「異業界プロとの出会いが事業の確変を起こす」） |
| 期間 | 開始〜終了（例: 2023/01〜2025/04） |
| 登場人物 | 関与した人物・組織 |
| 経緯 | 何がきっかけで・どう動いたか |
| 結果 | 事業への具体的インパクト（売上・採用・効率化等） |
| 成功/失敗 | 成功 or 失敗 |
| 再現性 | 同じパターンを繰り返すべきか・条件は何か |
| 関連シグナルID | Signal DBとのリンク |

**初期インプット方針（Phase 2-10 参照）**
- BUN社長が手動で主要ストーリーを入力（目標10〜20件）
- GitHubのStory_DB.mdとして保存し、COOが全ワークフローから参照できるようにする

**COOによる自動抽出（Signal DB蓄積後）**
- 週次バッチでSignal DBを分析し「ストーリー昇格候補」をBUN社長にLINEで提案
- BUN社長が「採用」とLINEで返信 → Story DBに自動追記
- BUN社長が「却下」とLINEで返信 → Signal DB上のアクション状態を「却下」に更新
...
| Googleカレンダー新規イベント検知時 | 会議前ブリーフィング（Story DB参照・関連ストーリー提示） | Claude Sonnet 4.6 | GPT-4o mini | LINE通知（会議30分前） |
| LINE会話受信時（常時） | BUN社長↔COO会話の自動記録化（Gemini判定→Signal DB蓄積） | Gemini 2.5 Flash | なし | Google Drive / Reports / BizDev / Signal_DB.csv |
| 事業イベント報告受信時（随時） | リアルタイム戦略更新評価（失注・受注・顧客変動を戦略的影響度評価） | Claude Opus 4.6 | GPT-4o mini | LINE通知（即時）+ Google Drive / Reports / BizDev / |
| 毎週日曜 4:00 | Signal DB週次クラスタリング（Story DB昇格候補の抽出・社長へLINE提案） | Claude Sonnet 4.6 | なし | LINE通知（社長への提案） |
...
BUN社長とCOO（n8nボット）のLINEやり取りは、口頭会議と同等の意思決定記録として扱い、自動でSignal DBに蓄積する。テキストの会話は音声よりAI処理精度が高く、BUN社長がLINEでCOOと対話するほど精度が向上するポジティブループが生まれる。

**フロー:**
1. BUN社長がLINEでCOOに指示・相談・報告を送信
2. n8n LINE WebhookがメッセージをリアルタイムでGemini 2.5 Flashに渡す
3. Gemini 2.5 Flashが「経営判断・意思決定・事業に関係する内容か」を判定
4. 関連する内容はランク付きでSignal_DB.csvに自動追記
5. 毎週日曜4:00の週次バッチでStory DB昇格候補を抽出・社長にLINEで提案
6. BUN社長が採用/却下をLINEで返信 → Story DBに自動反映

**保存先:** Google Drive / Reports / BizDev / Signal_DB.csv

### 会議前ブリーフィング（Story DB参照）

Googleカレンダーに新規ミーティングが登録された際、COOが関連するStory DBを参照し、「このミーティングに関係するBUN社長の過去ストーリーと推奨アクション」を会議30分前にLINEで提示する。会議の準備時間を削減しながら、過去の成功パターンを活かした立ち回りを実現する。

**LINEで届く内容:**
- 関連ストーリー: 類似ケースのタイトルと結果サマリー
- 推奨アクション: 過去の成功パターンに基づく会議でのポジション・提案方針
- 関連シグナル: 直近Signal DBから該当案件に関連する情報

**n8n実装:**
- トリガー: Google Calendar新規イベント作成 Webhook
- 処理: Claude Sonnet 4.6がStory_DB.mdとSignal_DB.csvを参照して会議ブリーフを生成
- 送信: LINE Messaging APIでBUN社長に通知（会議時刻の30分前にスケジュール送信）

### リアルタイム戦略更新フロー

BizDevや顧客対応における重要ビジネスイベント（失注・受注・顧客からの要望・競合動向）が発生した際、COOが即座に戦略的影響を評価してBUN社長に届ける。これにより経営戦略のリアルタイムアップデートが可能になる。

**フロー:**
1. 重要ビジネスイベント発生（LINEでCOOへ報告 or 自動検知）
2. Claude Opus 4.6が「現在の事業戦略に影響するか」を評価
3. 評価結果（影響度・推奨アクション・Story DB類似ケース）をLINEでBUN社長に即時送信
4. BUN社長の判断をSignal DBに記録 → 将来のStory DBに昇格

**評価内容:**
- このイベントは現在の事業戦略（OPS自動化・North Star OS・BizDev）に影響するか（Yes/No + 理由）
- 影響がある場合の推奨アクション（今日中 / 今週中 / 今月中 / 中長期）
- 過去の類似ストーリーとの比較（Story DB参照）

**保存先:** Google Drive / Reports / BizDev / StrategyUpdate_YYYYMMDD.md
...
├── Story_DB.md（BUN社長ストーリーDB・Phase 2-10で作成）
...
│   ├── BizDev/（事業機会レポート・Signal_DB.csv・StrategyUpdate_YYYYMMDD.md）
│   ├── FIN/（月次コスト・収支レポート）
│   └── DEV/（開発進捗レポート）
├── 🏢【KENZAI】給与計算/
...
└── LINEアプリ（通知受信・承認・COOへの指示・Signal DB蓄積の起点）
...
| Google Drive | データ（入力Excel・出力CSV・ダッシュボード・リサーチレポート・Signal_DB.csv） | 毎日生成される成果物・社長が見るファイル |
| GitHub | 設計書・マニュアル・コードのバックアップ・Story_DB.md | バージョン管理・COOのコンテキスト参照 |
...
| 2-9 | Signal DB構築（Google Drive / Reports / BizDev / Signal_DB.csv への自動蓄積フロー） |
| 2-10 | Story DB初期インプット（BUN社長が手動入力・目標10〜20件・GitHub / Story_DB.md として保存） |
| 2-11 | BUN社長クローン機能テスト（COOシステムプロンプトにデジタルクローン原則を統合・精度検証） |
| 2-12 | 会議前ブリーフィングワークフロー構築（Googleカレンダートリガー→Story DB参照→LINE送信 会議30分前） |
| 2-13 | LINE会話自動記録化ワークフロー構築（LINE Webhook受信→Gemini Flash判定→Signal DB書き込み） |
| 2-14 | リアルタイム戦略更新フロー構築（重要ビジネスイベント報告→Claude Opus評価→LINE即時通知） |
...
- Signal DBに1日平均50件以上のシグナルが自動蓄積されている
- Story DBにBUN社長の主要ストーリーが10件以上登録されている
- 会議前ブリーフィングが主要ミーティングの30分前にLINEに届いている
- BUN社長↔COOのLINE会話が自動でSignal DBに記録されている
